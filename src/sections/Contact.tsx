...
export const Contact = () => {
  ...
  const onSubmit: SubmitHandler<ContactFormData> = async (data) => {
    try {
      ...
      await emailjs.send('service_ppiz6aj', 'template_0igmfqr', (data as unknown) as Record<string, unknown>, {
  ...
  return (
    <section id="contact" className='min-h-screen flex justify-between items-center gap-8'>
      ...
      >
        ...
        >
          {isLoading ? (
            <span className="inline-block h-6 w-6 animate-spin rounded-full border-2 border-fuchsia-700 border-t-transparent" />
          ) : (
            'Send'
          )}
        </button>
